...
class MDNesterov(Optimizer):
    ...
    (batch, mini-batch)Stochastic gradient descent:
        ...
        $$
    """
    def __init__(self, params, lr=required, momentum = 0.1, weight_decay=0, BreDivFun ='Squared norm'):
        ...
        defaults = dict(lr=lr, momentum=momentum, weight_decay=weight_decay, BreDivFun = BreDivFun)
        super(MDNesterov, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(MDNesterov, self).__setstate__(state)
    ...
    def _search_param(self, param, param_lr_grad, BreDivFun='Squared norm', norm_p = 0):
        ...
        #     param = torch.muldiv(new_param_data, torch.rsqrt(torch.ones_like(new_param_data) + torch.pow(new_param_data, 2))
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                param_state['momentum_buffer'] = self._search_param(param = param_state['momentum_buffer'], param_lr_grad = d_p.mul(group['lr']), BreDivFun=BreDivFun)
                p.data.mul_(1 - momentum).add_(param_state['momentum_buffer'].mul(momentum))
